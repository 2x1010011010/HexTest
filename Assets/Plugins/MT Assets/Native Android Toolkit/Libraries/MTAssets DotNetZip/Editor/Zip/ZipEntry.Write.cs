...
namespace MTAssets.NativeAndroidToolkit.Editor.IonicDotNetZip.Zip
{
    public partial class ZipEntry
    {
        ...
        private bool WantReadAgain()
        {
            ...
            if (CompressionLevel == IonicDotNetZip.Zlib.CompressionLevel.None) return false;
        ...
        private void MaybeUnsetCompressionMethodForWriting(int cycle)
        {
            ...
            if (CompressionLevel == (short)IonicDotNetZip.Zlib.CompressionLevel.None &&
                CompressionMethod == IonicDotNetZip.Zip.CompressionMethod.Deflate)
                _CompressionMethod = 0x00;
        ...
        internal void WriteHeader(Stream s, int cycle)
        {
            ...
            // LastMod
            _TimeBlob = IonicDotNetZip.Zip.SharedUtilities.DateTimeToPacked(LastModified);
        ...
        private Int32 FigureCrc32()
        {
            if (_crcCalculated == false)
            {
                ...
                {
                    var output = new IonicDotNetZip.Crc.CrcCalculatorStream(Stream.Null);
                ...
                else
                {
                    ...
                    var crc32 = new IonicDotNetZip.Crc.CRC32();
        ...
        private void _WriteEntryData(Stream s)
        {
            ...
            try
            {
                ...
                var output = new IonicDotNetZip.Crc.CrcCalculatorStream(compressor, true);
        ...
        internal void FinishOutputStream(Stream s,
                                         ...
                                         Stream encryptor,
                                         Stream compressor,
                                         IonicDotNetZip.Crc.CrcCalculatorStream output)
        {
            ...
            if ((compressor as IonicDotNetZip.Zlib.DeflateStream) != null)
                compressor.Close();
#if BZIP
...
#if !NETCF
            else if ((compressor as IonicDotNetZip.Zlib.ParallelDeflateOutputStream) != null)
                compressor.Close();
#endif
            ...
            if (_UncompressedSize == 0 && _CompressedSize == 0)
            {
                ...
                if (_Password != null)
                {
                    ...
                    if (Encryption != EncryptionAlgorithm.None)
                    {
                        ...
                        // workitem 10178
                        IonicDotNetZip.Zip.SharedUtilities.Workaround_Ladybug318918(s);
        ...
        internal void PrepOutputStream(Stream s,
                                       ...
                                       out IonicDotNetZip.Crc.CrcCalculatorStream output)
        {
            ...
            // Wrap a CrcCalculatorStream around that.
            // This will happen BEFORE compression (if any) as we write data out.
            output = new IonicDotNetZip.Crc.CrcCalculatorStream(compressor, true);
        }
        ...
        {
            if (_CompressionMethod == 0x08 && CompressionLevel != IonicDotNetZip.Zlib.CompressionLevel.None)
            {
#if !NETCF
                     ...
                     _container.ParallelDeflateThreshold > 0L))
                {
                    ...
                    if (_container.ParallelDeflater == null)
                    {
                        _container.ParallelDeflater =
                            new IonicDotNetZip.Zlib.ParallelDeflateOutputStream(s,
                                                                       CompressionLevel,
                                                                       _container.Strategy,
                                                                       true);
                    ...
                    IonicDotNetZip.Zlib.ParallelDeflateOutputStream o1 = _container.ParallelDeflater;
                    o1.Reset(s);
                    return o1;
                }
#endif
                var o = new IonicDotNetZip.Zlib.DeflateStream(s, IonicDotNetZip.Zlib.CompressionMode.Compress,
                                                     CompressionLevel,
                                                     true);
                if (_container.CodecBufferSize > 0)
